...
def accuracy(output, target, topk=(1, )):
    ...
    _, pred = output.topk(maxk, 1, True, True) # (k, dim=None, largest=True, sorted=True, out=None), sorted 为_True_，将会确保返回的 k 个值被排序
    pred = pred.t()
...
def train(train_loader, model, criterion, optimizer, epoch, writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1) # (NN参数，最大梯度范数，范数类型=2)
