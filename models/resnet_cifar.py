...
################ 与论文相符的实现#####################
class BasicBlockWithStochasticDepth(nn.Layer):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, option='A', block_dpr=0.):
        super(BasicBlockWithStochasticDepth, self).__init__()
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                """
                self.shortcut = nn.Sequential(
                    nn.AvgPool2D(kernel_size=2), ###第一次修改：注意这里用平均池化来2倍下采样
...
##################与论文作者称道的Pytorch实现版本相同的实现################
class BasicBlockWithDeathRate(nn.Layer):
    expansion = 1
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', block_dpr=0.):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                """
                self.shortcut = nn.Sequential([
                    nn.AvgPool2D(kernel_size=2), ###注意这里用平均池化来2倍下采样
                    LambdaLayer(lambda x:
                                    F.pad(x,
                                        [0, 0, 0, 0, (planes - x.shape[2])//2, (planes - x.shape[2])//2], "constant", 0, data_format='NCDHW'))
                ])
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if self.b.sample().item()==1.:
                out = F.relu(self.bn1(self.conv1(x)))
                out = self.bn2(self.conv2(out))
                out = identity + out/(1-self.block_dpr) # 该版本在训练阶段通过主支时，输出除以保留率
            else:
                out = identity
        else:
            # 评估时保留full depth，利用集成效果，该版本block的主支输出不需要乘以保留率
            out = F.relu(self.bn1(self.conv1(x)))
            out = self.bn2(self.conv2(out))
            out += identity
        out = F.relu(out)
...
def resnet110(checkpoint=None, drop_path_rate=0.5, use_drop_path=True, use_official_implement=True):
    if use_official_implement:
        # 使用与论文和官方实现相符的版本 https://github.com/yueatsprograms/Stochastic_Depth 
        # https://github.com/shamangary/Pytorch-Stochastic-Depth-Resnet
        BasicBlock = BasicBlockWithStochasticDepth
    else:
        # 使用与论文作者称道的Pytorch实现版本相同的实现 https://github.com/felixgwu/img_classification_pk_pytorch
        BasicBlock = BasicBlockWithDeathRate

    model = ResNet(BasicBlock, [18, 18, 18], use_drop_path=use_drop_path, drop_path_rate=drop_path_rate)
